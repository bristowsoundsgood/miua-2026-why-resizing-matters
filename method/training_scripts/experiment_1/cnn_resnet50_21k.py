...
from resources.big_transfer.models import KNOWN_MODELS
...
## 1. LOAD MODEL ##
weights = np.load('resources/big_transfer/BiT-M-R50x1.npz')
RESNET50_21K = KNOWN_MODELS['BiT-M-R50x1']()
RESNET50_21K.load_from(weights)
...
for param in RESNET50_21K.parameters():        # Freeze all layers aside from classifier (fine-tuned on bus dataset)
    param.requires_grad = False
    
# Change classifier head to suit problem domain (1 class prediction 0->1)
torch.manual_seed(RANDOM_SEED)
new_classifier = nn.Conv2d(in_channels=2048, out_channels=1, kernel_size=1, stride=1)
RESNET50_21K.head.conv = new_classifier
...
optimiser = torch.optim.Adam(params=RESNET50_21K.parameters(), lr=LR_MAXIMUM)
...
checkpoint_path = os.path.join(CURRENT_DIR, "trained_checkpoints", "resnet50_21k.pt")
...
for epoch in tqdm(range(MAX_EPOCHS), desc='TRAINING'):
    ...
    with torch.inference_mode():        
        ...
        if early_stopper.check_early_stop(model=RESNET50_21K, val_loss=val_loss) == True:
...
RESNET50_21K.load_state_dict(state_dict=torch.load(early_stopper.path))
...
torch.save(obj=RESNET50_21K.state_dict(), f='trained_models/resnet50_21k.pt')

testing_metrics.print_performance()
testing_metrics.export_to_csv(filename='results/metrics_test_resnet50_21k')
training_metrics.export_to_csv(filename='results/metrics_train_resnet50_21k')