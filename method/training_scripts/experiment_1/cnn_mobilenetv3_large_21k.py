...
## 1. LOAD MODEL ##
MOBILENET_V3_21K = timm.create_model('mobilenetv3_large_100_miil_in21k', pretrained=True)

# The GitHub for this model (https://github.com/Alibaba-MIIL/ImageNet21K/blob/main/src_files/data_loading/data_loader.py) reveals that no normalization occurs, only resizing
preprocess_transform = T.Compose([
    T.ToTensor()
])

print(f'******* RAW PRE-TRAINED MODEL *******')
summary(model=MOBILENET_V3_21K,
...
dir_train_dataset = "bus_dataset/train"
dir_val_dataset = "bus_dataset/val"
dir_test_dataset = "bus_dataset/test"
...
for param in MOBILENET_V3_21K.parameters():
    param.requires_grad = False
    
# Change classifier head to suit problem domain (1 class prediction 0->1)
torch.manual_seed(RANDOM_SEED)
new_classifier = nn.Linear(in_features=MOBILENET_V3_21K.get_submodule('classifier').in_features, out_features=1)
MOBILENET_V3_21K.classifier = new_classifier
...
optimiser = torch.optim.Adam(params=MOBILENET_V3_21K.parameters(), lr=LR_MAXIMUM)
...
checkpoint_path = os.path.join(CURRENT_DIR, "trained_checkpoints", "__mobilenetv3_large_21k.pt")
...
for epoch in tqdm(range(MAX_EPOCHS), desc='TRAINING'):
    ...
    for X_train, y_train in train_dataloader:
        ...
        y_logits = MOBILENET_V3_21K(X_train)
    ...
    with torch.inference_mode():        
        for X_val, y_val in val_dataloader:
            ...
            y_logits = MOBILENET_V3_21K(X_val)
        ...
        if early_stopper.check_early_stop(model=MOBILENET_V3_21K, val_loss=val_loss) == True:
...
MOBILENET_V3_21K.load_state_dict(state_dict=torch.load(early_stopper.path))
...
with torch.inference_mode():
    ...
    for X_test, y_test in tqdm(test_dataloader, desc='TESTING'):
        ...
        y_logits = MOBILENET_V3_21K(X_test)
...
torch.save(obj=MOBILENET_V3_21K.state_dict(), f='trained_models/__mobilenet_v3_large_21k.pt')

testing_metrics.print_performance()
testing_metrics.export_to_csv(filename='results/__metrics_test_mobilenet_v3_21k')
training_metrics.export_to_csv(filename='results/__metrics_train_mobilenet_v3_21k')