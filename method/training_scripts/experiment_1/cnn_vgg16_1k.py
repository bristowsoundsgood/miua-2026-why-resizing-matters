...
VGG16_1K = torchvision.models.vgg16(weights=pretrained_weights)
...
for param in VGG16_1K.parameters():                 # Freeze all layers aside from classifier (fine-tuned on bus dataset)
...
VGG16_1K.classifier = new_classifier
...
optimiser = torch.optim.Adam(params=VGG16_1K.parameters(), lr=LR_MAXIMUM)
...
checkpoint_path = os.path.join(CURRENT_DIR, "trained_checkpoints", "vgg16_1k.pt")
...
for epoch in tqdm(range(MAX_EPOCHS), desc='TRAINING'):
    ...
    with torch.inference_mode():        
        ...
        if early_stopper.check_early_stop(model=VGG16_1K, val_loss=val_loss) == True:
...
VGG16_1K.load_state_dict(state_dict=torch.load(early_stopper.path))
...
torch.save(obj=VGG16_1K.state_dict(), f='trained_models/vgg16_1k.pt')

testing_metrics.print_performance()
testing_metrics.export_to_csv(filename='results/metrics_test_vgg16_1k')
training_metrics.export_to_csv(filename='results/metrics_train_vgg16_1k')