...
VIT_L_16_1K = torchvision.models.vit_l_16(weights=pretrained_weights)

print(f'******* RAW PRE-TRAINED MODEL *******')
summary(model=VIT_L_16_1K,
...
for param in VIT_L_16_1K.parameters():
...
VIT_L_16_1K.heads = new_classifier
...
optimiser = torch.optim.Adam(params=VIT_L_16_1K.parameters(), lr=LR_MAXIMUM)
...
checkpoint_path = os.path.join(CURRENT_DIR, "trained_checkpoints", "vit_l_16_1k.pt")
...
for epoch in tqdm(range(MAX_EPOCHS), desc='TRAINING'):
    ...
    with torch.inference_mode():        
        ...
        if early_stopper.check_early_stop(model=VIT_L_16_1K, val_loss=val_loss) == True:
...
VIT_L_16_1K.load_state_dict(state_dict=torch.load(early_stopper.path))
...
torch.save(obj=VIT_L_16_1K.state_dict(), f='trained_models/vit_l_16_1k.pt')

testing_metrics.print_performance()
testing_metrics.export_to_csv(filename='results/metrics_test_vit_l_16_1k')
training_metrics.export_to_csv(filename='results/metrics_train_vit_l_16_1k')