...
RESNET50_1K = torchvision.models.resnet50(weights=pretrained_weights)
...
for param in RESNET50_1K.parameters():
    param.requires_grad = False
    
# Change classifier head to suit problem domain (1 class prediction 0->1)
torch.manual_seed(RANDOM_SEED)
new_classifier = nn.Linear(in_features=RESNET50_1K.get_submodule('fc').in_features, out_features=1)
RESNET50_1K.fc = new_classifier
...
optimiser = torch.optim.Adam(params=RESNET50_1K.parameters(), lr=LR_MAXIMUM)
...
checkpoint_path = os.path.join(CURRENT_DIR, "trained_checkpoints", "resnet50_1k.pt")
...
for epoch in tqdm(range(MAX_EPOCHS), desc='TRAINING'):
    ...
    with torch.inference_mode():        
        ...
        if early_stopper.check_early_stop(model=RESNET50_1K, val_loss=val_loss) == True:
...
RESNET50_1K.load_state_dict(state_dict=torch.load(early_stopper.path))
...
torch.save(obj=RESNET50_1K.state_dict(), f='trained_models/resnet50_1k.pt')

testing_metrics.print_performance()
testing_metrics.export_to_csv(filename='results/metrics_test_resnet50_1k.csv')
training_metrics.export_to_csv(filename='results/metrics_train_resnet50_1k.csv')