...
## 1. LOAD MODEL ##
pretrained_weights = torchvision.models.Inception_V3_Weights.DEFAULT
preprocess_transform = pretrained_weights.transforms()
INCEPTIONV3_1K = torchvision.models.inception_v3(weights=pretrained_weights)
INCEPTIONV3_1K.aux_logits = False
...
for param in INCEPTIONV3_1K.parameters():                 # Freeze all layers aside from classifier (fine-tuned on bus dataset)
    param.requires_grad = False
    
# Change classifier head to suit problem domain (1 class prediction 0->1)
torch.manual_seed(RANDOM_SEED)
new_classifier = nn.Linear(
    in_features=INCEPTIONV3_1K.fc.in_features,
    out_features=1
)

INCEPTIONV3_1K.fc = new_classifier
...
optimiser = torch.optim.Adam(params=INCEPTIONV3_1K.parameters(), lr=LR_MAXIMUM)
...
checkpoint_path = os.path.join(CURRENT_DIR, "trained_checkpoints", "inceptionv3_1k.pt")
...
for epoch in tqdm(range(MAX_EPOCHS), desc='TRAINING'):
    ...
    for X_train, y_train in train_dataloader:
        ...
        y_logits = INCEPTIONV3_1K(X_train)
    ...
    with torch.inference_mode():        
        ...
        if early_stopper.check_early_stop(model=INCEPTIONV3_1K, val_loss=val_loss) == True:
...
INCEPTIONV3_1K.load_state_dict(state_dict=torch.load(early_stopper.path))
...
with torch.inference_mode():
    ...
    for X_test, y_test in tqdm(test_dataloader, desc='TESTING'):
        ...
        y_logits = INCEPTIONV3_1K(X_test)
...
torch.save(obj=INCEPTIONV3_1K.state_dict(), f='trained_models/inceptionv3_1k.pt')

testing_metrics.print_performance()
testing_metrics.export_to_csv(filename='results/metrics_test_inceptionv3_1k')
training_metrics.export_to_csv(filename='results/metrics_train_inceptionv3_1k')