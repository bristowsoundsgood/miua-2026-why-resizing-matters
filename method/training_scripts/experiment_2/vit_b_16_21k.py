...
from classes.vit_models import ViTWithCustomClassifier
...
]) # this transform is essentially the same as ViTImageProcessor.from_pretrained('google/vit-base-patch16-224-in21k'), but with the basic resizing method.
...
for param in pretrained_model.parameters():                 # Freeze all layers aside from classifier (fine-tuned on bus dataset)
    param.requires_grad = False
    
# Change classifier head to suit problem domain (1 class prediction 0->1)
torch.manual_seed(RANDOM_SEED)
classifier = nn.Linear(in_features=768, out_features=1)
    
VIT_B_16_21K = ViTWithCustomClassifier(model=pretrained_model, classifier=classifier)

print(f'******* ADAPTED MODEL FOR FINE-TUNING *******')
summary(model=VIT_B_16_21K,
        input_size=(1, 3, 224, 224),
        col_names=['input_size', 'output_size', 'num_params', 'trainable'],
        col_width=20,
        row_settings=['var_names'])
...
optimiser = torch.optim.Adam(params=VIT_B_16_21K.parameters(), lr=LR_MAXIMUM)
...
checkpoint_path = os.path.join(CURRENT_DIR, "trained_checkpoints", "vit_b_16_21k.pt")
...
for epoch in tqdm(range(MAX_EPOCHS), desc='TRAINING'):
    ...
    with torch.inference_mode():        
        ...
        if early_stopper.check_early_stop(model=VIT_B_16_21K, val_loss=val_loss) == True:
...
VIT_B_16_21K.load_state_dict(state_dict=torch.load(early_stopper.path))
...
torch.save(obj=VIT_B_16_21K.state_dict(), f='trained_models/vit_b_16_21k.pt')

testing_metrics.print_performance()
testing_metrics.export_to_csv(filename='results/metrics_test_vit_b_16_21k.csv')
training_metrics.export_to_csv(filename='results/metrics_train_vit_b_16_21k.csv')