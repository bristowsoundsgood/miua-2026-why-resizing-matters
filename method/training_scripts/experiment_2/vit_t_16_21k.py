...
]) # this transform is essentially the same as ViTImageProcessor.from_pretrained('WinKawaks/vit-tiny-patch16-224k'), but with the basic resizing method.
...
classifier = nn.Linear(in_features=192, out_features=1)
    
VIT_T_16_21K = ViTWithCustomClassifier(model=pretrained_model, classifier=classifier)

print(f'******* ADAPTED MODEL FOR FINE-TUNING *******')
summary(model=VIT_T_16_21K,
        input_size=(1, 3, 224, 224),
        col_names=['input_size', 'output_size', 'num_params', 'trainable'],
        col_width=20,
        row_settings=['var_names'])
...
optimiser = torch.optim.Adam(params=VIT_T_16_21K.parameters(), lr=LR_MAXIMUM)
...
checkpoint_path = os.path.join(CURRENT_DIR, "trained_checkpoints", "vit_t_16_21k.pt")
...
for epoch in tqdm(range(MAX_EPOCHS), desc='TRAINING'):
    ...
    with torch.inference_mode():        
        ...
        if early_stopper.check_early_stop(model=VIT_T_16_21K, val_loss=val_loss) == True:
...
VIT_T_16_21K.load_state_dict(state_dict=torch.load(early_stopper.path))
...
torch.save(obj=VIT_T_16_21K.state_dict(), f='trained_models/vit_t_16_21k.pt')

testing_metrics.print_performance()
testing_metrics.export_to_csv(filename='results/metrics_test_vit_t_16_21k.csv')
training_metrics.export_to_csv(filename='results/metrics_train_vit_t_16_21k.csv')